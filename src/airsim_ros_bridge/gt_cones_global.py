...
def main():
    ...
    while not rospy.is_shutdown():
      ...
      for blue_cone in gt_blue_cones:
        lm = Landmark()
        lm.position.x = blue_cone[0]
        lm.position.y = blue_cone[1]
        lm.type = 0
        lms.landmarks.append(lm)

      for yellow_cone in gt_yellow_cones:
        lm = Landmark()
        lm.position.x = yellow_cone[0]
        lm.position.y = yellow_cone[1]
        lm.type = 1
        lms.landmarks.append(lm)
      
      for large_cone in gt_large_cones:
        lm = Landmark()
        lm.position.x = large_cone[0] 
